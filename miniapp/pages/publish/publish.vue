  ...
  <view class="p-30">
    ...
    <dd-card padding="0 30rpx" width="630rpx" margin="30rpx 0 0 0">
      <view class="py-30 border-b flex flex-between item-center">
        <view class="publish-label fo-28 bold"> 物品描述: </view>
        <view class="publish-content fo-28">
          {{ goodsDesc }}
        </view>
      </view>
      <view class="py-30 border-b flex flex-between item-center">
        <view class="publish-label fo-28 bold"> 配送路程: </view>
        <view class="publish-content fo-28">
          {{ distanceLabel }}
        </view>
      </view>
      ...
      <view class="py-30 flex flex-between item-center">
        <view class="publish-label fo-28 bold"> 物品重量: </view>
        <view class="publish-content fo-28">
          {{ weightLabel }}
        </view>
      </view>
    ...
    <dd-card padding="0 30rpx" width="630rpx" margin="30rpx 0 0 0">
      <view
        class="py-30 border-b flex flex-between item-center"
        @click="navToCoupon"
      >
        ...
        <view class="publish-content flex flex-between item-center">
          <view v-if="userCouponId === '' && ableCoupons > 0" class="fo-28">
            您有
            <text class="fo-info bold">{{ ableCoupons }}</text> 张可用优惠券
          </view>
          <view
            v-else-if="userCouponId === '' && ableCoupons === 0"
            class="fo-28 fo-9"
          >
            暂无可用优惠券
          </view>
      ...
      <view class="py-30 flex flex-between item-center">
        ...
        <view
          class="publish-content fo-28 flex flex-between item-center"
          @click="doUseIntegral()"
        >
          <view v-if="canChangeAmount > 0" class="fo-28">
            您可使用{{ useIntegral }}积分,抵扣<text class="fo-info">{{
              canChangeAmount
            }}</text
            >元
          </view>
          <view v-else class="fo-28"> 您的积分不足于抵扣 </view>
          <label>
            <checkbox
              :checked="useIntegralPay"
              :disabled="canChangeAmount === 0"
            />
          </label>
        </view>
      </view>
    ...
    <view class="fo-24 fo-9 mt-16"> 优惠券与积分只能选则一项 </view>
      ...
      <dd-card padding="0 30rpx" width="630rpx" margin="30rpx 0 0 0">
        <view class="py-30 flex flex-between item-center">
          ...
          <view class="publish-content fo-28 flex flex-between item-center">
            ...
            <view v-else class="fo-28 fo-3"> {{ feeTips[tipCurrent] }}元 </view>
            <view class="iconfont icon-arrow-right fo-28 fo-9"></view>
          </view>
        </view>
      </dd-card>
    ...
    <view class="bottom-fix flex flex-between item-center">
      <view class="fo-36 bold ml-32"> 需付款{{ totalPrice }}元 </view>
      <view class="publish-button" @click="publish()"> 立即下单 </view>
    </view>
  </view>
</template>
...
import { BUY_NEARBY, HELP_BUY } from "../../util/constant.js";
export default {
  components: {
    AddressShow,
  },
  ...
  methods: {
    ...
    async getIntegral() {
      ...
      if (result.code === 200) {
        ...
        this.withIntegral = result.data.config
          ? result.data.config.withIntegral
          : 10000;
    ...
    getChangeAmount() {
      const amount =
        Math.floor(
          Math.round((this.integralBalance / this.withIntegral) * 100)
        ) / 100;
      if (this.originPrice - amount < 0) {
        this.useIntegral =
          this.integralBalance -
          (amount - this.originPrice) * this.withIntegral;
    ...
    async calculation() {
      ...
      const result = await post("order/calculation", {
        startAddress:
          home.buyStyle === BUY_NEARBY && home.tabCurrent === HELP_BUY
            ? undefined
            : home.startAddress,
    ...
    async publish() {
      ...
      uni.requestSubscribeMessage({
        tmplIds: [orderTmpId, orderTmpId, orderTmpId],
        complete: async () => {
          uni.showLoading({
            title: "正在创建订单..",
          });
          ...
          const params = {
            ...
            serviceType: home.tabCurrent,
            userCouponId:
              this.userCouponId === "" ? undefined : this.userCouponId,
          };
          if (this.tipCurrent > 0) {
                ...
                uni.requestPayment({
                  ...
                  complete: (res) => {
                    ...
                    const timeout = setTimeout(() => {
                      uni.redirectTo({
                        url:
                          "/pages/order/detail/detail?orderNo=" +
                          result.data.orderNo,
                      });
