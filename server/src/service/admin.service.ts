...
export class AdminService extends BaseService {
  ...
  async findByNo(adminNo: string) {
    return await this.adminEntity.findOne({ where: { adminNo } });
  }
  ...
  // 是否有超级管理员
  async hasSuperAdmin() {
    return !!(await this.adminEntity.findOne({ where: { superAdmin: true } }));
  }
  ...
  async list(adminListDTO: AdminListDTO) {
    ...
    return await this.queryService.select(this.adminEntity, {
      ...
      wheres,
      fields:
        'adminNo,adminName,mobileNumber,realName,status,createTime,updateTime,defaultPwd, updatedBy',
