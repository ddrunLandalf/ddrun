<template>
  <div class="find-user">
    ...
    >
      <a-avatar v-if="userInfo.userNo" :src="userInfo.avatarUrl" :size="40">
        <i slot="icon" class="iconfont icon-avatar"></i>
      </a-avatar>
      <div v-if="userInfo.userNo" class="ml-12">
        <div class="fo-14">
          {{ userInfo.nickName }}
          <span v-if="userInfo.mobileNumber">({{ userInfo.mobileNumber }})</span>
          <span class="fo-14 fo-9">NO: {{ userInfo.userNo }}</span>
        </div>
      </div>
    </div>
  </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async searchUsers(params: any) {
      this.loading = true;
      const item = await (this as any).$api.userList(params);
      this.loading = false;
      if (item.code === 200) {
        if (item.data.count > 0) {
          this.userInfo = item.data.data[0];
          this.$emit('change', this.userInfo.userNo);
        } else {
